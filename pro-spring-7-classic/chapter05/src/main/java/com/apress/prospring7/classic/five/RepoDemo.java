...
import com.apress.prospring7.classic.five.repo.SingerJdbcRepo;
import com.apress.prospring7.classic.five.repo.SingerRepo;
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RepoDemo.class);
     ...
     static void main() {
        try(final var ctx = new AnnotationConfigApplicationContext(C3p0DataSourceCfg.class, SingerJdbcRepo.class)) {
            ...
            LOGGER.info("--------------------------");
            singerRepo.findNameById(1L).ifPresentOrElse(s ->  LOGGER.info("Result: {}", s),
                    () -> LOGGER.info("No record found with id: " + 1L) );
