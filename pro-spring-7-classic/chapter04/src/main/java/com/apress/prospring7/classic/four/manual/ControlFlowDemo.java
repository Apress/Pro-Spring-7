...
public class ControlFlowDemo {
 ...
 public void run() {
  final var pc = new ControlFlowPointcut(ControlFlowDemo.class, "test");
  final var advisor = new DefaultPointcutAdvisor(pc, new LogAroundAdvice());

  final var pf = new ProxyFactory();
  pf.setTarget(new FooTarget());
  pf.addAdvisor(advisor);

  final var proxy = (FooTarget) pf.getProxy();
  LOGGER.info("\tInvoke in main method");
  proxy.foo();
  LOGGER.info("\tInvoke in ControlFlowDemo.test(..)");
  test(proxy);
 }

 private void test(final FooTarget proxy) {
  proxy.foo();
 }
}
