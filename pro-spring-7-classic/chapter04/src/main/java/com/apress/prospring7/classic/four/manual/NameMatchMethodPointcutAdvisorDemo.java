...
public class NameMatchMethodPointcutAdvisorDemo {
 ...
 public static void main(String... args) {
  final var advisor = new NameMatchMethodPointcutAdvisor(new LogAroundAdvice());
  advisor.setMappedNames("foo", "qux");

  final var  pf = new ProxyFactory();
  pf.setTarget(new GooTarget());
  pf.addAdvisor(advisor);

  final var proxy = (GooTarget) pf.getProxy();
  proxy.foo();
  proxy.foo(new Bar());
  proxy.qux();
  System.out.println("--------------------");
