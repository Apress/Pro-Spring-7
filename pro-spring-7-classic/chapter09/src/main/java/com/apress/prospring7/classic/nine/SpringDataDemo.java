...
import com.apress.prospring7.classic.nine.services.SingerService;
...
public class SpringDataDemo {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SpringDataDemo.class);

    static void main() {
        try (var ctx = new AnnotationConfigApplicationContext(DataJpaCfg.class)) {
            var service = ctx.getBean(SingerService.class);

            LOGGER.info(" ---- Testing the stored procedure ------");
            LOGGER.info("-- Singer with id {}  has firstName {} ", 1L, service.getFirstNameByIdProc(1L));
        }
    }
}
