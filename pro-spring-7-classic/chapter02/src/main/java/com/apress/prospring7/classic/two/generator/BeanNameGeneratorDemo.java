...
SOFTWARE.
*/
package com.apress.prospring7.classic.two.generator;
...
class SimpleBeanNameGenerator extends AnnotationBeanNameGenerator {
    ...
    @Override
    protected String buildDefaultBeanName(BeanDefinition definition, final BeanDefinitionRegistry registry) {
        var beanName = definition.getBeanClassName().substring(definition.getBeanClassName().lastIndexOf(".") + 1).toLowerCase(Locale.ROOT);
        var uid = UUID.randomUUID().toString().substring(0,8).replace("-","");
        return beanName + "-" + uid;
    }
}
